...
from scripts.lib import closing_soon, market_watchlist, query_type, schema, sports_schedule
        ...
        "competitions": [
            {
                ...
                "competitors": [
                    {"homeAway": "home", "score": "51", "team": {"displayName": f"Home {league_title}"}},
                    {"homeAway": "away", "score": "47", "team": {"displayName": f"Away {league_title}"}},
                ],
            }
        ],
    }
...
class ClosingSoonTests(unittest.TestCase):
    ...
    def test_live_sports_label_requires_matching_market(self):
        ...
        events = [
            _market_event("lal-hou", "Lakers vs. Rockets", "2026-04-20T05:30:00Z"),
            _market_event("lal-hou-series", "NBA Playoffs: Who Will Win Series? - Lakers vs. Rockets", "2026-05-04T00:00:00Z"),
        ...
        self.assertEqual(live_items[0]["title"], "Lakers vs. Rockets")
