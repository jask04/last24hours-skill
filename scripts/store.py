#!/usr/bin/env python3
"""SQLite research accumulator for last24hours.
...
Database location: ~/.local/share/last24hours/research.db
"""
...
DB_DIR = Path.home() / ".local" / "share" / "last24hours"
...
def update_finding(finding_id: int, **kwargs):
    ...
    try:
        ...
        if invalid_columns:
            raise ValueError(f"Invalid finding update fields: {', '.join(invalid_columns)}")
        sets = ", ".join(f"{k} = ?" for k in kwargs)
...
def main():
    parser = argparse.ArgumentParser(description="Query the last24hours research database")
