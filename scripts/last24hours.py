...
def main():
    ...
    parser.add_argument(
        ...
        choices=range(1, 3),
        metavar="N",
        help="Number of days to look back (1-2, default: 1)",
    )
    parser.add_argument(
        "--store",
        action="store_true",
        help="Persist findings to SQLite database (~/.local/share/last24hours/research.db)",
    )
    parser.add_argument(
        "--diagnose",
        action="store_true",
    ...
    if args.debug:
        os.environ["LAST24HOURS_DEBUG"] = "1"
    ...
    if args.diagnose:
        ...
        diag = {
            ...
            "polymarket": True,
            "kalshi": True,
            "sportsbook": env.is_sportsbook_available(config),
            "sportsbook_monthly_usage": list(sportsbook.current_month_usage()) if env.is_sportsbook_available(config) else None,
            "casino_reference": env.is_casino_context_available(),
    ...
    closing_seed_cap = 3 if kalshi_fast_scan else 8 if args.paper_fast_watchlist else 12
        ...
        if not live_sports_mode and preferred_closing_venue != "polymarket":
            try:
                ...
                closing_raw_ka = closing_soon.scan_kalshi_closing_soon(
                    ...
                    max_candidates=4 if kalshi_fast_scan else 16 if args.paper_fast_watchlist else 25,
                    raw_cap_per_seed=10 if kalshi_fast_scan else 24 if args.paper_fast_watchlist else 40,
