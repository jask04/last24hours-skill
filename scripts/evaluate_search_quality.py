...
def create_eval_env(include_web: bool) -> Tuple[Dict[str, str], Path]:
    ...
    eval_home = Path(tempfile.mkdtemp(prefix="last24hours-eval-home-"))
    ...
    passthrough = {
        ...
        "PYTHONUTF8": "1",
        "LAST24HOURS_CONFIG_DIR": "",
        "BIRD_DISABLE_BROWSER_COOKIES": "1",
        "LAST24HOURS_DISABLE_BROWSER_COOKIES": "1",
    }
...
def create_worktree(rev: str) -> Path:
    worktree_dir = Path(tempfile.mkdtemp(prefix="last24hours-eval-"))
    subprocess.run(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--timeout", type=int, default=180, help="Per-topic timeout passed to last24hours")
...
def main() -> int:
    ...
    try:
        for topic, query_type in topics:
            slug = slugify(topic)
            baseline_report, baseline_stderr = run_last24hours(
                baseline_dir,
                topic,
                search=args.search,
            ...
            candidate_report, candidate_stderr = run_last24hours(
                candidate_dir,
                topic,
                search=args.search,
                timeout_seconds=args.timeout,
