"""Instagram Reels search via ScrapeCreators API for /last24hours.
...
def search_instagram(
    ...
    try:
        ...
        url = f"{SCRAPECREATORS_BASE}/v2/instagram/reels/search"
...
def fetch_captions(
    ...
    for item in top_items:
        ...
        cached = cache.get_granular_cache("instagram", vid)
        if cached:
            captions[vid] = cached
            continue
        ...
        try:
            params = {"url": url}
            api_url = f"{SCRAPECREATORS_BASE}/v2/instagram/media/transcript"
            headers = _sc_headers(token)
            ...
            if transcripts and isinstance(transcripts, list):
                ...
                if transcript_text:
                    ...
                    cache.set_granular_cache("instagram", vid, transcript_text)
        except Exception as e:
