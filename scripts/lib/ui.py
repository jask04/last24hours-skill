"""Terminal UI utilities for last24hours skill."""
...
POLYMARKET_MESSAGES = [
    ...
    "Checking market attention...",
    "Scanning Polymarket probabilities...",
...
Just start with "last24" and talk to me like normal.
"""
...
To fix this:
1. Add AUTH_TOKEN and CT0 to ~/.config/last24hours/.env or .claude/last24hours.env
...
def show_diagnostic_banner(diag: dict):
    ...
    if IS_TTY:
        ...
        lines.append(f"{Colors.DIM}│{Colors.RESET} {Colors.BOLD}/last24hours v1.0.59 — Source Status{Colors.RESET}                    {Colors.DIM}│{Colors.RESET}")
        ...
        if has_openai:
            ...
            lines.append(f"{Colors.DIM}│{Colors.RESET}     └─ Add to ~/.config/last24hours/.env            {Colors.DIM}│{Colors.RESET}")
        ...
        lines.append(f"{Colors.DIM}│{Colors.RESET}                                                     {Colors.DIM}│{Colors.RESET}")
        lines.append(f"{Colors.DIM}│{Colors.RESET}  Config: {Colors.BOLD}~/.config/last24hours/.env{Colors.RESET}                  {Colors.DIM}│{Colors.RESET}")
        lines.append(f"{Colors.DIM}└─────────────────────────────────────────────────────┘{Colors.RESET}")
    else:
        # Plain text for non-TTY (Claude Code / Codex)
        lines.append("┌─────────────────────────────────────────────────────┐")
        lines.append("│ /last24hours v1.0.59 — Source Status                    │")
        lines.append("│                                                     │")
        ...
        if has_openai:
            ...
            lines.append("│     └─ Add to ~/.config/last24hours/.env            │")
        ...
        lines.append("│                                                     │")
        lines.append("│  Config: ~/.config/last24hours/.env                  │")
        lines.append("└─────────────────────────────────────────────────────┘")
